...
function setTool(tool) {
  ...
  document.querySelector('.canvas-main').classList.toggle('eraser-mode', tool === 'eraser');
}
...
async function generateAI() {
  ...
  // Build prompt: drawing as image + description
  ...
  try {
    ...
    // We also send the canvas as an image data URL for reference in prompt
    const canvasDataUrl = drawCanvas.toDataURL('image/png');
    ...
    const body = {
      ...
      options: {
        wait_for_model: true, // don't error if model is loading
        use_cache:      false,
      },
    };
    ...
    const errMsg = err.message.includes('503')
      ? 'Model is loading (cold start) — wait 20s and try again.'
      : err.message.includes('429')
      ? 'Rate limited by Hugging Face — wait a minute and try again.'
      : err.message.includes('401') || err.message.includes('403')
      ? 'Unauthorized — check your HF token, or leave blank for free (slower) access.'
      : err.message;
...
// Build a text prompt from user description + drawing analysis
function buildPrompt(desc) {
  const base = desc
    ? `${desc}, digital art, highly detailed, vibrant colors, artstation trending`
    : 'abstract colorful digital painting, vibrant, highly detailed, artstation trending';

  return `${base}, 4k resolution, beautiful composition, creative art`;
}
